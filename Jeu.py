...
@st.cache_data(ttl=5) # Rafraîchissement rapide pour le prof
...
if role == "Professeur":
    ...
    with c1:
        ...
        except: st.info("Aucun score enregistré.")
    with c2:
...
# --- SECTION ETUDIANT ---
    ...
    if not nom_utilisateur:
        ...
        st.metric("Ma position réelle", f"Case {curr_pos} / {max_c}")
        st.divider()

        # ÉTAPE 1 : LANCER LE DÉ
        ...
        # ÉTAPE 2 & 3 : QUESTION OU CASE LIBRE + RÉSULTAT
        else:
            t_pos = st.session_state.temp_pos
            st.subheader(f"🚀 Tu vises la case : {t_pos}")
            
            # On vérifie s'il y a une question sur cette case
            ...
            if not q_data.empty:
                        ...
                        if st.form_submit_button("Valider"):
                            ...
                            try:
                                ...
                                if nom_utilisateur in df_s["Etudiant"].values:
                                    ...
                                    df_s = pd.concat([df_s, pd.DataFrame([{"Etudiant": nom_utilisateur, "Position": nouvelle_p}])], ignore_index=True)
                                conn.update(spreadsheet=URL_SCORES, worksheet=instance, data=df_s)
                            except: pass

                            st.session_state.rep_validee = True
                            st.session_state.res = (juste, bonne, nouvelle_p)
                            st.rerun()
                else:
                    # AFFICHAGE DU RESULTAT APRES VALIDATION
                ...
                # CASE SANS QUESTION
                st.warning("🍃 Case libre ! Pas de question ici.")
                if st.button("S'installer sur cette case"):
                    try:
                        ...
                        if nom_utilisateur in df_s["Etudiant"].values:
                            ...
                            df_s = pd.concat([df_s, pd.DataFrame([{"Etudiant": nom_utilisateur, "Position": t_pos}])], ignore_index=True)
                        conn.update(spreadsheet=URL_SCORES, worksheet=instance, data=df_s)
                    except: pass
                    del st.session_state.temp_pos
                    st.rerun()
